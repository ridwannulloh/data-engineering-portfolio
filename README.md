...
### Project 6: portfolio_dbt — Analytics Engineering on DuckDB
**Description:** Self-contained dbt project demonstrating a full **seed → staging → marts** analytics pipeline on DuckDB, runnable end-to-end with a single `docker compose up`. Raw CSV seeds emulate an AWS DMS / S3 landing zone (Bronze/ODS), staging views type-cast and convert timestamps to WIB, and mart tables denormalise orders against customers, products, and lookup seeds into a BI-ready fact plus a customer RFM summary. Originally modelled on Amazon Redshift, then ported to DuckDB so the entire pipeline runs locally with zero cloud credentials.

**Technologies:** dbt, DuckDB, Docker, Docker Compose, SQL, Python

**Key Features:**
- Layered dbt architecture: `raw` seeds → `staging` views → `marts` tables, with literal schema naming via a custom `generate_schema_name`
- Self-contained & reproducible — raw data shipped as deterministic CSV seeds (regenerated by `scripts/gen_seed_data.py`), no external data source needed
- `mart_orders` wide fact (orders × customers × products + country/status lookups) and `mart_customer_summary` with RFM-style recency segmentation
- Custom Jinja macros: `to_wib()` UTC→Asia/Jakarta conversion, `jakarta_today()`, MD5 `surrogate_key()`
- 48 data tests (`unique`, `not_null`, `accepted_values`, plus a custom `non_negative` generic test) and source freshness checks
- Hermetic build — no external dbt packages, so the container needs no `dbt deps` / network
- Fully containerised: `docker compose up --build` runs seed → run → test; a `docs` service serves the lineage graph at `http://localhost:8080`

**[View Project →](./dbt_duckdb)**

---

### Project 7: Real-Time E-Commerce Intelligence Platform
**Description:** End-to-end streaming + batch data platform that turns raw e-commerce activity into live business intelligence. Three synthetic producers stream orders, warehouse inventory, and user clickstream into Kafka; PyFlink computes real-time analytics (hourly GMV per sales channel via tumbling windows, and stockout-risk alerts via sliding windows); a Bronze sink lands raw events to a MinIO lakehouse, and batch pipelines refine them through a Medallion Architecture (Bronze → Silver → Gold) into analytics-ready tables — hourly GMV and quintile-based **Customer RFM segmentation**. Fully containerised: one `docker compose up --build` boots the entire platform — Kafka, MinIO, Flink, producers, and the Bronze→Silver→Gold loop — and you watch the lakehouse fill in real time.

**Technologies:** Apache Kafka, PyFlink (Flink 1.18), MinIO (S3), Apache Airflow, Parquet, boto3, pandas, Docker Compose, pytest

**Key Features:**
- One-command turnkey demo — `docker compose up --build` runs infrastructure *and* app services with health-gated startup (no manual steps, no cloud account, no API keys)
- Dual processing models: **real-time** PyFlink windows (tumbling GMV, sliding stockout-risk alerts) alongside **batch** medallion refinement
- Medallion lakehouse on a real object store: partitioned NDJSON Bronze (`year=/month=/day=/hour=`) → cleaned Parquet Silver → analytical Gold, all browsable in the MinIO console
- Data-quality enforcement: quarantine pattern for malformed records (never silently dropped), null checks, deduplication, and lineage metadata on every record
- Customer RFM segmentation using quintile scoring (Champions, Loyal, At Risk, Lost …) — robust to skewed distributions vs. fixed thresholds
- Operational alerting pattern: per-product sliding-window stockout detection by velocity, not just point-in-time stock levels
- Small-file-aware Bronze sink (batched flushes), environment-driven config that runs identically on host or in containers, and a production-style Airflow DAG
- 18 pytest unit tests covering all Silver/Gold transformation logic — no external services required

**[View Project →](./realtime-ecommerce-pipeline)**
